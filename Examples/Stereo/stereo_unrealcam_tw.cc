...
{  
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            // downscale imLeft
            cv::Mat imLeftCopy = imLeft.clone();
            cv::resize(imLeftCopy, imLeft, cv::Size(545, 407), cv::INTER_CUBIC);

            // crop and upscale imRight
            cv::Rect ROI(545, 407, 545, 407); // (X, Y, Width, Height)
            imRight = imRight(ROI);
